...
初始化完成后，可直接访问 `http://服务器地址:8180/` 查看公开用量状态。该页面无需登录，会显示连接名称、掩码邮箱、套餐、额度、重置时间和数据状态；不会公开完整邮箱、账号 ID、原始同步错误或 Token 历史。管理员登录入口为 `/login`，登录后的总览为 `/overview`；旧 `/public` 链接会重定向到根路径。公开页部署后始终可用，如不希望对外展示，请在反向代理层限制根路径和 `/api/v1/public/overview` 的访问。
