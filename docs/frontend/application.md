...
总览通过顶部选择器显示当前账号名称和掩码邮箱。统一的余额面板在标题区显示套餐与同步更新时间，在内容区按窗口展示剩余百分比、窗口时长和重置时间；多窗口使用同一面板内的响应式网格，缺失限额时仍保留面板元信息并显示明确空状态。四项摘要与每日 Token 图位于余额面板之后。“历史天数”使用每日 Token 趋势的日桶数量，空历史显示 `0 天`，并与 Telegram 历史概览保持一致。`usedPercent` 展示前限制到 0–100，但原始数据语义不得在 API 类型层改写。缺失摘要显示“暂无”。
