...
总览按去除首尾空白且忽略大小写的邮箱分组，同邮箱的个人订阅和所有 Team / Business 工作区进入同一卡片；没有邮箱的连接按账号 ID 各自成卡。卡片保留未连接、待识别、失败和无额度状态，每个窗口紧凑显示剩余百分比、窗口时长、重置时间和剩余时间。总览只自动读取内存快照，不触发 Codex 同步；“查看详情”会保存连接 ID 并进入详情页。邮箱必须保持掩码，百分比只在展示时限制到 0–100，不对不同连接的额度进行数学相加。

详情通过顶部选择器显示当前账号名称和掩码邮箱。刷新操作使用保留动态无障碍名称的方形图标按钮；移动端选择器和刷新按钮保持单行排列，并维持至少 `44px` 的触控区域。统一的余额面板在标题区显示套餐与同步更新时间，在内容区按窗口展示剩余百分比、窗口时长和重置时间。窗口标题由 `windowDurationMinutes` 格式化，不向用户暴露内部 `primary`/`secondary` 位置标识，周期缺失时显示“限额窗口”。四项摘要与每日 Token 图位于余额面板之后。“历史天数”使用每日 Token 趋势的日桶数量，空历史显示 `0 天`，并与 Telegram 历史概览保持一致。缺失摘要显示“暂无”。
